...
ENGINES = {
    "chat": [
        ...
        "3.1-sonar-large-128k-online",
        "3.1-sonar-huge-128k-online",
    ],
}
...
def run(**kwargs) -> Tuple[Optional[str], Optional[Dict[str, Any]], Any, Any]:
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "system",
                "content": kwargs.get("system_prompt", "Be precise and concise."),
            },
            {"role": "user", "content": prompt},
        ],
        "max_tokens": kwargs.get(
            "max_tokens", DEFAULT_PERPLEXITY_SETTINGS["max_tokens"]
        ),
        "temperature": kwargs.get(
            "temperature", DEFAULT_PERPLEXITY_SETTINGS["temperature"]
        ),
        "top_p": kwargs.get("top_p", DEFAULT_PERPLEXITY_SETTINGS["top_p"]),
        "search_recency_filter": kwargs.get(
            "search_recency_filter",
            DEFAULT_PERPLEXITY_SETTINGS["search_recency_filter"],
        ),
        "presence_penalty": kwargs.get(
            "presence_penalty", DEFAULT_PERPLEXITY_SETTINGS["presence_penalty"]
        ),
        "frequency_penalty": kwargs.get(
            "frequency_penalty", DEFAULT_PERPLEXITY_SETTINGS["frequency_penalty"]
        ),
        "stream": kwargs.get("stream", DEFAULT_PERPLEXITY_SETTINGS["stream"]),
        "return_images": kwargs.get(
            "return_images", DEFAULT_PERPLEXITY_SETTINGS["return_images"]
        ),
        "return_related_questions": kwargs.get(
            "return_related_questions",
            DEFAULT_PERPLEXITY_SETTINGS["return_related_questions"],
        ),
        "search_domain_filter": kwargs.get(
            "search_domain_filter", DEFAULT_PERPLEXITY_SETTINGS["search_domain_filter"]
        ),
    }
